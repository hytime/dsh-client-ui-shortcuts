...
export function ShortcutBindingEditor({ bindings, availableGlobalActions, t, onChange, platform, disabled = false, onValidityChange, query }: ShortcutBindingEditorProps): React.ReactElement {
  ...
  const displayed = useMemo(() => visible.filter(({ binding }) => {
    if (normalizedQuery === '') return true
    const commandName = t(`keyboard.${binding.command}`).toLowerCase()
    const scopeName = t(`legend.scope.${binding.scope}`).toLowerCase()
    return commandName.includes(normalizedQuery) || scopeName.includes(normalizedQuery) || binding.command.toLowerCase().includes(normalizedQuery)
  }), [normalizedQuery, t, visible])
